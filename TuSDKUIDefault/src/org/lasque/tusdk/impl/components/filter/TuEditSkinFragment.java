/** 
 * TuSDKCore
 * TuEditSkinFragment.java
 *
 * @author 		Clear
 * @Date 		2015-4-22 下午5:35:53 
...
public class TuEditSkinFragment extends TuEditSkinFragmentBase
{
	...
	{
		return TuSdkContext.getLayoutResId("tusdk_impl_component_edit_skin_beauty_fragment");
	}

	/** 美肤编辑功能控制器 */
	public TuEditSkinFragment()
	{
...
//			super.onParameterConfigRest(this.getConfigView(), this.getCurrentAction());
	...
	protected View buildActionButton(String type, int index)
	{
		...
		String icon = String.format("lsq_style_default_edit_icon_%s", type).toLowerCase();
	...
	{
		if (this.getActionsWrap() == null) return;
		
		View btn = this.getActionsWrap().findViewWithTag(0);
		setEnabled(btn, found);
		
		btn = this.getActionsWrap().findViewWithTag(4);
		setEnabled(btn, found);
		
		btn = this.getActionsWrap().findViewWithTag(5);
		setEnabled(btn, found);
	}
