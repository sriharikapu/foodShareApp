...
package com.fan.framework.base.UI.drag_sort;
...
public class DragSortListView extends ListView {
     ...
     * the dragged item_image_gallery.
     */
     ...
     * View. If dropped, the dragged item_image_gallery will land in this position.
     */
    ...
    /**
     * Offset (in x) within the dragged item_image_gallery at which the user
     * picked it up (or first touched down with the digitalis).
     */
    private int mDragDeltaX;

    /**
     * Offset (in y) within the dragged item_image_gallery at which the user
     * picked it up (or first touched down with the digitalis).
     */
    ...
    /**
     * A listener that receives a callback when the floating View
     * (or more precisely the originally dragged item_image_gallery) is removed
     * by one of the provided gestures.
     */
    private RemoveListener mRemoveListener;

    /**
     * Enable/Disable item_image_gallery dragging
     *
    ...
    /**
     * Height in pixels to which the originally dragged item_image_gallery
     ...
     * position. Remove in favor of upper-bounding item_image_gallery position?
     */
     ...
     * position. Remove in favor of lower-bounding item_image_gallery position?
     */
     ...
     * a sliding item_image_gallery (only used if drag-sort animations
     * are turned on). Nearly 1 means the item_image_gallery is
     * at the top of the slide region (nearly full blank item_image_gallery
     * is directly below).
     */
     ...
     * item_image_gallery View given by the user inside another View (currenly
     * a RelativeLayout) which
     * expands and collapses to simulate the item_image_gallery shuffling.
     */
    ...
    /**
     * Needed for adjusting item_image_gallery heights from within layoutChildren
     */
     ...
     * know the height of an offscreen item_image_gallery. Since ListView virtualizes
     * these, DSLV must get the item_image_gallery from the ListAdapter to obtain
     * its height. That process can be expensive, but often the same
     * offscreen item_image_gallery will be requested many times in a row. Once an
     * offscreen item_image_gallery height is calculated, we cache it in this guy.
     * Actually, we cache the height of the child of the
     * DragSortItemView since the item_image_gallery height changes often during a
     * drag-sort.
     */
     ...
     *
     * @return The ListAdapter set as the argument of {@link ()}
     */
    ...
    private class AdapterWrapper extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            // Set the correct item_image_gallery height given drag state; passed
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mDragState != IDLE) {
            // draw the divider over the expanded item_image_gallery
    ...
    private int getItemHeight(int position) {
        ...
        if (v != null) {
            // item_image_gallery is onscreen, just get the height of the View
            return v.getHeight();
        } else {
            // item_image_gallery is offscreen. get child height and calculate
            // item_image_gallery height based on current shuffle state
    ...
    private class HeightCache {
        ...
        /**
         * Add item_image_gallery height at position if doesn't already exist.
         */
    ...
    /**
     * Get the shuffle edge for item_image_gallery at position when top of
     * item_image_gallery is at y-coord top. Assumes that current item_image_gallery heights
     ...
     *
     * @param position
     * @param top      at position. If -1, this function
     *                 calculates this height.
     * @return Shuffle line between position-1 and position (for
     * the given view of the list; that is, for when top of item_image_gallery at
    ...
    private int getShuffleEdge(int position, int top) {
        ...
        // first calculate top of item_image_gallery given that floating View is
    ...
    /**
     * Removes an item_image_gallery from the list and animates the removal.
     *
    ...
    /**
     * Move an item_image_gallery, bypassing the drag-sort process. Simply calls
    ...
    /**
     * Cancel a drag. Calls {@link #(boolean, boolean)} with
    ...
    /**
     * Removes dragged item_image_gallery from the list. Calls RemoveListener.
     */
    ...
    private void adjustOnReorder() {
        ...
        if (mSrcPos < firstPos) {
            // collapsed src item_image_gallery is off screen;
            // adjust the scroll after item_image_gallery heights have been fixed
            View v = getChildAt(0);
            int top = 0;
            if (v != null) {
                top = v.getTop();
     ...
     * like to remove the dragged item_image_gallery from the list.
     *
     * @param remove Remove the dragged item_image_gallery from the list. Calls
    ...
    private void adjustItem(int position, View v, boolean invalidChildHeight) {
        ...
        // Adjust item_image_gallery gravity
        ...
        // Finally adjust item_image_gallery visibility
    ...
    private int getChildHeight(int position) {
        ...
        if (v != null) {
            // item_image_gallery is onscreen, therefore child height is valid,
            // hence the "true"
            return getChildHeight(position, v, false);
        } else {
            // item_image_gallery is offscreen
    ...
    private int calcItemHeight(int position, int childHeight) {
        ...
        if (position == mSrcPos) {
            if (mSrcPos == mFirstExpPos) {
                ...
                // if gets here, we know an item_image_gallery is sliding
            ...
            // we know an item_image_gallery is sliding (b/c 2ndPos != 1stPos)
    ...
    /**
     * Start a drag of item_image_gallery at <code>position</code> using the
    ...
    /**
     * Start a drag of item_image_gallery at <code>position</code> without using
     * a FloatViewManager.
     *
     * @param position  Item to drag.
    ...
    public boolean startDrag(int position, View floatView, int dragFlags, int deltaX, int deltaY) {
        ...
        // set src item_image_gallery invisible
    ...
    private void updateFloatView() {
        ...
        final int numHeaders = getHeaderViewsCount() + (has ? 1 : 0);
        final int numFooters = getFooterViewsCount();
        final int firstPos = getFirstVisiblePosition();
        ...
        // bottom of last item_image_gallery in list
    ...
    public interface FloatViewManager {
        /**
         * Return the floating View for item_image_gallery at <code>position</code>.
         ...
         *
         * @param position Position of item_image_gallery to drag (NOTE:
     ...
     * When an item_image_gallery removal occurs, DragSortListView
    ...
    /**
     * Use this to move the check state of an item_image_gallery from one position to another
     ...
     * stable IDs, ListView will attempt to locate each item_image_gallery based on its ID and
     * move the check state from the item_image_gallery's old position to the new position 鈥�
     ...
     * close neighborhood of the old position. If the user moves an item_image_gallery too far
     * (specifically, more than 20 rows away), ListView will give up and just
     * force the item_image_gallery to be unchecked. So if there is a reasonable chance that
    ...
    public void moveCheckState(int from, int to) {
        ...
        // runs. Note that moving an item_image_gallery from A to B is essentially a rotation
        ...
        // X X . X X X . X
        // we have two runs. One begins at the last item_image_gallery of the list and wraps
        ...
        if (runCount == 1 && (runStart[0] == runEnd[0])) {
            ...
            // item_image_gallery to false like we do below.
            return;
        }
    ...
    /**
     * Use this when an item_image_gallery has been deleted, to move the check state of all
    ...
    private class DragScroller implements Runnable {
        ...
        public void run() {
            ...
            // dy is change in View position of a list item_image_gallery; i.e. positive dy
            // means user is scrolling up (list item_image_gallery moves down the screen,
            // remember
