...
public class ZoomImageView extends AppCompatImageView {
    ...
    public void setImageBitmap(final Bitmap bitmap) {
        ...
        if (getWidth() <= 0) {
            final int bitmapHashCode = bitmapHash;
            post(new Runnable() {
                @Override
                public void run() {
                    if (bitmapHashCode != bitmapHash) {
                        return;
                    }
        ...
        arithScaleRate();
        zoomTo(scaleRate, FFUtils.getDisWidth() / 200f, FFUtils.getDisHight() / 200f);
        center(true, true);
    }
        ...
        mHandler.post(new Runnable() {
            public void run() {
                ...
                if (currentMs < durationMs) {
                    ...
                    if (Math.abs(target - mMinZoom) < 0.2) {
                        mHandler.post(new Runnable() {
                            @Override
                            public void run() {
                                zoomTo(mMinZoom, centerX, centerY);
                                mHandler.post(new Runnable() {
                                    @Override
                                    public void run() {
                                        zoomTo(mMinZoom, centerX, centerY);
