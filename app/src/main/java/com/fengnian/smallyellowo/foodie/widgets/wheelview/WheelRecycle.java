...
public class WheelRecycle {
	 ...
	 * @param firstItem the number of first item_image_gallery in layout
	 * @param range the range of current wheel items 
	 * @return the new value of first item_image_gallery number
	 */
	public int recycleItems(LinearLayout layout, int firstItem, ItemsRange range) {
		...
		for (int i = 0; i < layout.getChildCount();) {
			if (!range.contains(index)) {
				...
				if (i == 0) { // first item_image_gallery
					firstItem++;
				}
			} else {
				i++; // go to next item_image_gallery
			}
			index++;
		}
		return firstItem;
	...
	/**
	 * Gets empty item_image_gallery view
	...
	/**
	 * Adds view to cache. Determines view type (item_image_gallery view or empty one) by index.
	 * @param view the view to be cached
