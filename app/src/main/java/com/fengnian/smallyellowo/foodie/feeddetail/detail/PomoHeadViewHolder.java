...
public class PomoHeadViewHolder extends BaseHeadViewHolder {
    ...
    public PomoHeadViewHolder(DynamicDetailActivity activity, View headView) {
    ...
    protected void refresh1() {
        ...
        if (!TextUtils.isEmpty(title)) {
            if (title.length() >= 18) {
                tv_title.setTextSize(TypedValue.COMPLEX_UNIT_PX,
                        getActivity().getResources().getDimensionPixelSize(R.dimen.font_size_1));
            }
        }
        ...
        //人均
        if (getActivity().data.getFood().getTotalPrice() > 0 && getActivity().data.getFood() != null) {
    ...
    private int getItemWidth(final int index) {
        ...
        if (DisplayUtil.screenWidth <= 720) {
            itemWidth = DisplayUtil.dip2px(101f);
        } else {
            itemWidth = DisplayUtil.dip2px(130f);
        }
