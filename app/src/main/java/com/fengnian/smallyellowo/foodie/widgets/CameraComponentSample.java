...
import com.fengnian.smallyellowo.foodie.fragments.MyCameraOption;
 ...
 * @author Clear
 */
public class CameraComponentSample extends SampleBase implements MyCameraFragment.TuCameraFragmentDelegate {
    /**
     * 相机组件范例
     */
    public CameraComponentSample() {
        super(R.string.lsq_album_alert_title);
    }
    ...
    public void showSample(Activity activity) {
        ...
        MyCameraOption option = new MyCameraOption();
...
//		 option.setGroupTableCellLayoutId(GroupFilterGroupView.getLayoutId());
        ...
        MyCameraFragment fragment = option.fragment();
...
//	    option.setMarkImage(BitmapHelper.getRawBitmap(activity, R.raw.sample_watermark));
     ...
     */
    @Override
    public void onTuCameraFragmentCaptured(final MyCameraFragment fragment, final TuSdkResult result) {
//        fragment.hubDismissRightNow();
//        fragment.dismissActivityWithAnim();
//        TLog.d("onTuCameraFragmentCaptured: %s", result);
        Log.i("tag","onTuCameraFragmentCaptured--"+System.currentTimeMillis());
        // 默认输出为 Bitmap  -> result.image  0: 原图;   1:带滤镜的图
        final Bitmap[] bitmaps = new Bitmap[]{result.bitmaps.get(0), result.bitmaps.get(1)};
        result.bitmaps.clear();
        final int id = MyProgressDialog.show(fragment.getActivity(), false, "正在保存...", 10000);
        new Thread() {
            @Override
            public void run() {
                final String[] paths = new String[2];
                String filterCode = result.filterCode;
                if (filterCode.equals("Normal")) {
                    filterCode = "";
                }
                boolean hasFilter = !FFUtils.isStringEmpty(filterCode);
                paths[0] = FileUitl.getCacheFileDir() + "/" + XData.uuid() + ".fast";
                FFImageUtil.saveBitmap(paths[0], bitmaps[0]);

                if (hasFilter) {
                    paths[1] = FileUitl.getCacheFileDir() + "/" + XData.uuid() + ".fast";
                    FFImageUtil.saveBitmap(paths[1], bitmaps[1]);
                }
                APP.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        fragment.addImage(bitmaps, paths, result.filterCode);
                        Log.i("tag","已保存--"+System.currentTimeMillis());
                        APP.showToast("已保存", null);
                        MyProgressDialog.Dismiss(fragment.getActivity(), id);
                    }
                });
            }
        }.start();
        fragment.getCamera().startCameraCapture();
...
//         option.setSaveToAlbum(true);  -> result.image
     ...
     */
    @Override
    public boolean onTuCameraFragmentCapturedAsync(MyCameraFragment fragment, TuSdkResult result) {
     ...
     */
    @Override
    public void onTuAlbumDemand(MyCameraFragment fragment) {
