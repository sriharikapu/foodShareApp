...
package com.fengnian.smallyellowo.foodie.zxing.view;
...
//    textPaint.setTextSize(TEXT_SIZE * density);
    ...
    mRect=new Rect();
    int left= Color.parseColor("#666666");
    int center=getResources().getColor(R.color.search_color);

//    int center=Color.parseColor("#2cb044");
    int right=Color.parseColor("#666666");
...
//      canvas.drawRect(frame.left + 2, middle - 1, frame.right - 1, middle + 2, paint);
      //meihua
      paint.setColor(getResources().getColor(R.color.search_color));
    //画出四个角
...
//        canvas.drawRect(-15 + (-50 + frame.right), -15+ ((0 - linewidht) + frame.bottom), -15 + frame.right, -15+ (linewidht - (linewidht - 1) + frame.bottom), paint);
    ...
    canvas.drawRect(5 + frame.left, 5 + frame.top,5 + (linewidht + frame.left), 5 + (50 + frame.top), paint);
    canvas.drawRect(5 + frame.left, 5 + frame.top,5 + (50 + frame.left), 5 + (linewidht + frame.top), paint);
    canvas.drawRect(-5 + ((0 - linewidht) + frame.right),5 + frame.top, -5 + (1 + frame.right),5 + (50 + frame.top), paint);
    canvas.drawRect(-5 + (-50 + frame.right), 5 + frame.top, -5+frame.right, 5 + (linewidht + frame.top), paint);


    canvas.drawRect(5 + frame.left, -5 + (-49 + frame.bottom),5 + (linewidht + frame.left), -5 + (1 + frame.bottom),    paint);
    canvas.drawRect(5 + frame.left, -5+ ((0 - linewidht) + frame.bottom), 5 + (50 + frame.left), -5 + (1 + frame.bottom), paint);
    canvas.drawRect(-5 + ((0 - linewidht) + frame.right), -5+ (-49 + frame.bottom), -5 + (1 + frame.right), -5+ (1 + frame.bottom), paint);
    canvas.drawRect(-5 + (-50 + frame.right), -5+ ((0 - linewidht) + frame.bottom), -5 + frame.right, -5+ (linewidht - (linewidht - 1) + frame.bottom), paint);



    int middle = frame.width() ;
    textPaint.setTextSize(60);
    textPaint.setColor(Color.WHITE);
    String text ="将取景框对准二维码,";
    String text1="即可自动扫描。";
    float text_width = textPaint.measureText(text);
      float text_width1 = textPaint.measureText(text1);
    int temp_x=(int) (middle-text_width)/2;

      int temp_x1=(int) (middle-text_width1)/2;
    int www=(CameraManager.get().getDis().x-middle)/2;

    canvas.drawText(text, www+temp_x , frame.bottom + 120, textPaint);
    canvas.drawText(text1, www+temp_x1 , frame.bottom + 220, textPaint);
