...
import com.fengnian.smallyellowo.foodie.feeddetail.detail.ChinaStyleHeadViewHolder;
...
public class RichContentChinaSnapAdapter extends SnapBaseAdapter {

    public RichContentChinaSnapAdapter(Activity activity, SYFeed data, String url) {
    ...
    public void getView(int position, final OnSnapViewCreatedListener listener) {
        switch (getItemViewType(position)) {
            ...
            return;
            case 1: {//图片
                final View convertView = activity.getLayoutInflater().inflate(R.layout.item_dynamic_detial_china_style_image, null, false);
            ...
            case 2: {//文字
                View convertView = activity.getLayoutInflater().inflate(R.layout.item_dynamic_detial_china_text, null, false);
                TextView tv_content = (TextView) convertView.findViewById(R.id.tv_content);
...
//        }
    ...
    private int getItemWidth(final int index) {
        ...
        if (DisplayUtil.screenWidth <= 720) {
            itemWidth = DisplayUtil.dip2px(121f);
        } else {
            itemWidth = DisplayUtil.dip2px(150f);
        }
